...
Example using "Vault" as secrets store

* See https://learn.hashicorp.com/collections/vault/getting-started
on how to setup Vault as your secrets store. Then use something like the following
...
Use Vault's ACLs to limit access of the token. Destroy the token after starting the service
during boot with systemd.
